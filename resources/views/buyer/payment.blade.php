...
<head>
  ...
  <title>Pembayaran Pesanan #{{ $order->order_number }} — Warung Mas Ndutt94</title>
...
<body class="custom-bg min-h-screen overflow-y-auto py-8 flex justify-center">
  ...
  <div class="main-content bg-white shadow-xl rounded-2xl p-8 w-full max-w-md text-center border-t-4 border-primary">
    ...
    <!-- Judul -->
    <h1 class="text-4xl font-black fredoka-font text-primary">Pembayaran Pesanan</h1>
    <p class="text-gray-600 text-sm mt-1">Selesaikan pembayaran untuk memproses pesanan</p>
    ...
    <div class="bg-gray-50 rounded-xl p-5 my-6 border border-primary/20 shadow-sm">
      ...
      <p class="font-mono text-xl font-black text-primary">{{ $order->order_number }}</p>
    ...
    <!-- Form -->
    <form id="paymentForm"
          action="{{ route('buyer.payment.confirm', ['order' => $order->id]) }}"
          method="POST"
      ...
      <div class="text-left space-y-3 mb-4">
        ...
        <div>
          ...
          <input type="text" name="customer_name"
                 value="{{ old('customer_name', $order->customer_name !== 'Antrian #' . $order->queue_number ? $order->customer_name : '') }}"
      ...
      <div class="space-y-4 text-left">
        ...
        <label class="block p-4 border rounded-xl bg-white cursor-pointer shadow-sm hover:border-primary transition">
          ...
          <p class="text-xs text-gray-500 mt-2">
            Upload bukti pembayaran setelah transfer/scan QRIS.
          </p>
        </label>
        ...
        <label class="block p-4 border rounded-xl bg-white cursor-pointer shadow-sm hover:border-primary transition">
          ...
          <p class="text-xs text-gray-500 mt-2">
            Bayar langsung di kasir warung.
          </p>
        </label>
      ...
      <div class="mt-4 text-left">
        <label class="block text-sm font-semibold text-gray-700 mb-1">
          Upload Bukti Pembayaran (khusus Transfer/QRIS)
        </label>
